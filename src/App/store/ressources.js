...
export const loadImages = createAsyncThunk(
  ...
  async () => {
    ...
    const pr2 = await Promise.all([
      await fetch(`${REST_ADR}${RESSOURCES_NAME.images}`),
      await fetch(`${REST_ADR}${RESSOURCES_NAME.memes}`)
    ])
    const [pri,prm]=pr2;
...
const ressourceReducer=(state = initialState, { type, payload }) => {
  switch (type) {

  case 'ressources/fillImages':
...
export default ressourceReducer;
export const fillImages=(payload)=>{
    return {type:'ressources/fillImages',payload:payload}
}
*/